...
*     http://ptrbrtz.net/portfolio/razor-ahrs/
...
// DEBUG OPTIONS
/*****************************************************************/
// When set to true, g_yro drift correction will not be applied
...
class DCM
{
    ...
    float qd[3];
		...
		float G_Dt = 0.01f; // Integration time for DCM algorithm
		
		void DCM_init(float Kp_RP, float Ki_RP, float Kp_Y, float Ki_Y);
    void setSensorVals(float g_x, float g_y, float g_z, float a_x, float a_y, float a_z, float m_x, float m_y, float m_z, float magHead);
	  ...
	  private:
		...
		float gyro_Vector[3]= {0, 0, 0}; // Store the g_yros turn rate in a vector
		float Omega_Vector[3]= {0, 0, 0}; // Corrected g_yro_Vector data
