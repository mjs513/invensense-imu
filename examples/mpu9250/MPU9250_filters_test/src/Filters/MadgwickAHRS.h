...
void MadgwickAHRSupdateIMU(float g_x, float g_y, float g_z, float a_x, 
						   float a_y, float a_z, float deltat)
{
  float recipNorm;
  float s0, s1, s2, s3;
...
void MadgwickAHRSupdate(float g_x, float g_y, float g_z, 
						...
						float m_x, float m_y, float m_z, float deltat)
{
	...
	if((m_x == 0.0f) && (m_y == 0.0f) && (m_z == 0.0f)) {
		MadgwickAHRSupdateIMU(g_x, g_y, g_z, a_x, a_y, a_z, deltat);
		return;
	}
