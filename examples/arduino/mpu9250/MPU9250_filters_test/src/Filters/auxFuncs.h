...
void getYawPitchRollRad(float * ypr) {
  float q[4];
  #if defined(use_DCM)
	dcm.getQ(&q[0], &q[1], &q[2], &q[3]);
  #elif defined(use_unavahrs)
  Filter.getQ(&q[0], &q[1], &q[2], &q[3]);
  #elif defined(use_dcmalgo)
  dcmAlgorithm.getQ(&q[0], &q[1], &q[2], &q[3]);
  #else
	getQ(&q[0], &q[1], &q[2], &q[3]);
  #endif
